...
const AboutAyurvedaPageDutch = () => {
    ...
    const [spicesRef, spicesInView] = useInView({
        triggerOnce: true, // Animation will only happen once
        threshold: 0.1,    // Trigger when 10% of the image is visible
    });

    const [gmaDRef, gmaDInView] = useInView({
        triggerOnce: true,
        threshold: 0.1,
    });

    const [gmaMRef, gmaMInView] = useInView({
        triggerOnce: true,
        threshold: 0.1,
    });

    const [womanStretchingRef, womanStretchingInView] = useInView({
        triggerOnce: true,
        threshold: 0.1,
    });

    const [soupWithSpicesRef, soupWithSpicesInView] = useInView({
        triggerOnce: true,
        threshold: 0.1,
    });

    const [asanaRef, asanaInView] = useInView({
        triggerOnce: true,
        threshold: 0.1,
    });
        ...
        <div className="flex flex-col items-center w-full">
                ...
                <div className="relative w-full md:w-3/5 self-center order-1 md:order-first mb-6 md:mb-0">
                    ...
                    <div className="w-full relative md:mt-6 md:ml-6 lg:mt-8 lg:ml-8">
                        ...
                        <div className="h-[250px] md:h-[500px] overflow-hidden">
                            <img 
                                ref={spicesRef} // New: Attach the ref
                                loading="lazy"
                                className={`w-full h-full object-cover object-[center_25%] md:object-center shadow-lg transition-opacity duration-1000 ease-out ${
                                    spicesInView ? 'opacity-100' : 'opacity-0'
                                }`} // New: Conditionally apply opacity
                                src={images.spices}
                                alt="Rijen schalen gevuld met kleurrijke kruiden zitten voor een kleine winkel op een markt" 
                            />
                        </div>
                    </div>
                </div>
            ...
            <div className="flex flex-col md:flex-row items-center bg-white w-full pt-4 md:py-0">
                ...
                <div className="w-full md:w-1/2 h-full">
                    <img 
                        ref={asanaRef} // Add this line
                        loading="lazy"
                        src={images.asana} 
                        className={`w-full h-auto md:h-full object-cover transition-opacity duration-1000 ease-out ${
                            asanaInView ? 'opacity-100' : 'opacity-0'
                        }`} // Update this line
                        alt="Een silhouet van een vrouw op een heuvel die een yogahouding doet bij zonsondergang."
                    />
                </div>
            </div>
            {/*Doshas section*/}
                ...
                <div className="flex flex-col md:flex-row md:gap-8 lg:gap-20 justify-center items-center">
                    ...
                    <div className="flex flex-col md:flex-row md:gap-6 lg:gap-10 w-full md:w-auto">
                        ...
                        <div className="flex flex-col items-center pb-8 w-full md:w-auto">
                            ...
                            <div className="w-full max-w-[280px] sm:max-w-[300px] md:w-auto lg:h-[450px] lg:w-[330px] p-2 md:p-4 border-2 border-black">
                                <img
                                    ref={gmaDRef} // New: Attach the ref
                                    loading="lazy"
                                    className={`w-full h-full object-cover object-top transition-opacity duration-1000 ease-out ${
                                        gmaDInView ? 'opacity-100' : 'opacity-0'
                                    }`} // New: Conditionally apply opacity
                                    src={images.gmaD}
                                    alt="Een zwart-wit afbeelding van een jonge vrouw met een denkend gezicht en kort haar dat gekruld is in een stijl uit de jaren 1950."
                                />
                            </div>
                        ...
                        <div className="flex flex-col items-center pb-8 w-full md:w-auto">
                            ...
                            <div className="w-full max-w-[280px] sm:max-w-[300px] md:w-auto lg:h-[450px] lg:w-[330px] p-2 md:p-4 border-2 overflow-hidden border-black">
                                <img
                                    ref={gmaMRef} // New: Attach the ref
                                    loading="lazy"
                                    className={`w-full h-full object-cover object-top transition-opacity duration-1000 ease-out ${
                                        gmaMInView ? 'opacity-100' : 'opacity-0'
                                    }`} // New: Conditionally apply opacity
                                    src={images.gmaM}
                                    alt="Een zwart-wit afbeelding van een vrouw van middelbare leeftijd met een vol gezicht en armen."
                                />
                            </div>
                            <div className="flex flex-col w-full max-w-[320px] sm:max-w-[400px] min-h-[320px] md:min-h-[375px] bg-white p-4 md:p-8 mt-4">
                                <p className="text-md">
                    ...
                    <div className="hidden md:flex items-center md:max-w-[15%]">
                        <img 
                            ...
                            className="max-h-[120px] lg:max-h-[200px] animate-[fade-in_1s_ease-out] [animation-fill-mode:forwards]"
            ...
            <div className="w-full bg-white flex flex-col md:flex-row items-center justify-center md:h-[40vh]">
                <div className="w-full md:w-auto flex justify-center md:h-full">
                    <img 
                        ref={womanStretchingRef}
                        src={images.meditate} 
                        className={`h-auto md:h-full object-contain transition-opacity duration-1000 ease-out ${
                            womanStretchingInView ? 'opacity-100' : 'opacity-0'
                        }`}
                        alt="Een vrouw zit te mediteren op een klein stukje grond omringd door water terwijl de zon ondergaat."
                    />
                </div>
                ...
                <div className="relative w-full md:w-4/5 lg:w-auto self-center mt-6 lg:mt-0">
                    ...
                    <div className="w-full md:w-auto lg:w-auto relative md:mt-4 md:mr-4 lg:mt-8 lg:mr-8">
                        <img 
                            ref={womanStretchingRef}
                            loading="lazy"
                            className={`w-full md:max-w-full lg:w-auto lg:max-h-[40rem] object-cover shadow-lg transition-opacity duration-1000 ease-out ${
                                womanStretchingInView ? 'opacity-100' : 'opacity-0'
                            }`}
                            src={images.womanStretching}
                            alt="Een man zit op een yogamat zich uit te rekken, er staan planten en boeken op de achtergrond"
                        />
                    </div>
                </div>
            </div>
            <div className="flex flex-col lg:flex-row items-center  lg:py-12 gap-8 lg:gap-12  md:px-6 lg:px-0">
                <div className="relative w-full md:w-4/5 lg:w-auto self-center order-last lg:order-first mt-6 lg:mt-0">
                    ...
                    <div className="w-full md:w-auto lg:w-auto relative md:mb-4 md:ml-4 lg:mb-8 lg:ml-8">
                        <img 
                            ref={soupWithSpicesRef}
                            loading="lazy"
                            className={`w-full md:max-w-full lg:w-auto lg:max-h-[800px] object-cover shadow-lg transition-opacity duration-1000 ease-out ${
                                soupWithSpicesInView ? 'opacity-100' : 'opacity-0'
                            }`}
                            src={images.soupWithSpices}
                            alt="Twee kommen pompoensoep, met feta en pompoenpitten erover gestrooid." 
                        />
                    </div>
                </div>
                <div className="flex flex-col w-full lg:max-w-[800px] lg:w-[80vw] px-4  lg:py-6 text-base md:text-lg lg:text-xl">
